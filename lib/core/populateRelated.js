...
module.exports = function populateRelated (docs, relationships, callback) {
	if (Array.isArray(docs)) {
		async.each(docs, function (doc, done) {
			doc.populateRelated(relationships).then(function (value) { done(null, value); }).catch(done);
		}, callback);
	} else if (docs && docs.populateRelated) {
		docs.populateRelated(relationships).then(function (value) { callback(null, value); }).catch(callback);
	} else {
		callback();
	}
	return this;
