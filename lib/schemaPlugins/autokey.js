...
module.exports = function autokey () {
	...
	if (autokey.unique) {
		def[autokey.path].index = { unique: true, sparse: true };
	}

	this.schema.add(def);

	var getUniqueKey = async function (doc, src) {
		...
		var results = await q.exec();

		// deliberate use of implicit type coercion with ==
		if (results.length && (results.length > 1 || results[0].id != doc.id)) {
			var inc = src.match(/^(.+)\-(\d+)$/);
			if (inc && inc.length === 3) {
				src = inc[1];
				inc = '-' + ((inc[2] * 1) + 1);
			...
			return getUniqueKey(doc, src + inc);
		} else {
	...
	this.schema.pre('save', async function () {
		...
		if (autokey.unique) {
			await getUniqueKey(this, newKey);
		} else {
