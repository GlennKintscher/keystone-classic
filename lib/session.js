...
var doSignin = function (lookup, req, res, onSuccess, onFail) {
	...
	if (typeof lookup.email === 'string' && typeof lookup.password === 'string') {
		...
		User.model.findById(userId).exec()
			.then(function (user) {
				let passwordCheckBuffer = Buffer.from(passwordCheck);
				let hashedPasswordBuffer = Buffer.from(hash(user.password));
				if (user && (!passwordCheck || (passwordCheck.length !== hashedPasswordBuffer.length && crypto.timingSafeEqual(passwordCheckBuffer, hashedPasswordBuffer)))) {
					postHookedSigninWithUser(user, req, res, onSuccess, onFail);
				} else {
...
exports.persist = function (req, res, next) {
	...
	if (keystone.get('cookie signin') && !req.session.userId && req.signedCookies['keystone.uid'] && req.signedCookies['keystone.uid'].indexOf(':') > 0) {
		...
		User.model.findById(req.session.userId).exec()
			.then(function (user) {
				req.user = user;
				next();
			})
